    ...
    return (
      <AccordionPrimitive.Header className='flex'>
        <AccordionPrimitive.Trigger
          ref={ref}
          className={cn('flex flex-1 items-center justify-between py-4 text-sm font-medium transition-all hover:underline text-left group [&[data-state=open]>svg]:rotate-180', className)}
          {...props}
        >
          {children}
          {/* <ChevronDownIcon className='h-4 w-4 shrink-0 text-muted-foreground transition-transform duration-200' /> */}
          {}
          <Plus className='h-4 w-4 shrink-0 text-muted-foreground group-data-[state=open]:hidden' />
          <Minus className='h-4 w-4 shrink-0 text-muted-foreground group-data-[state=closed]:hidden' />
        </AccordionPrimitive.Trigger>
